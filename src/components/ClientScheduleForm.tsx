...
export function ClientScheduleForm({ clientId, onSave }: ClientScheduleFormProps) {
  ...
  async function handleSave() {
    if (rules.length === 0) {
      setSnackbar({ message: 'Добавьте хотя бы одно правило расписания', type: 'error' });
      return;
    }
    ...
    try {
      ...
      setSnackbar({ message: 'Расписание сохранено', type: 'success' });
    } catch (error) {
      console.error('Error saving schedule:', error);
      setSnackbar({ message: 'Ошибка при сохранении расписания: ' + (error as Error).message, type: 'error' });
    } finally {
      setSaveStatus('idle');
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <Snackbar
        ...
        autoHideDuration={snackbar?.type === 'error' ? 5000 : 3000}
      />
